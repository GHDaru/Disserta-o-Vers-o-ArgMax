...
\begin{tikzpicture}[node distance=2cm, auto]
  % Nodes
  \node (consulta) [draw, rectangle] {consulta};
  \node (qi) [draw, rectangle, right=2.5cm of consulta] {\(q_i\)};
  \node (R) [draw, rectangle, right=2.5cm of qi] {\(R(q_i, d_j)\)};
  \node (/mathcal{C}) [draw, rectangle, right=of R] {\(c\)};
  \node (descriptions) [draw, rectangle, , below=1.5cm of consulta] {descrições};
  \node (documentos) [draw, rectangle, right=1.5cm of descriptions] {documentos};
  \node (dj) [draw, rectangle, right=of documentos] {\(d_j\)};
  ...
  \draw[-{Latex[length=3mm]}] (qi) -- (R);
  \draw[-{Latex[length=3mm]}] (dj) -- (R);
  \draw[-{Latex[length=3mm]}] (R) -- (consulta);
\end{tikzpicture}
...
% Florestas Aleatórias, introduzidas por Ho \cite{ho1995random}, são um método de aprendizado de máquina para classificação, regressão e outras tarefas, que operam construindo uma multitude de árvores de decisão no momento do treinamento e produzindo a classe que é o modo das classes (classificação) ou previsão média (regressão) das árvores individuais. Florestas aleatórias corrigem o hábito das árvores de decisão de se ajustarem demais ao seu conjunto de treinamento \cite{breiman2001random}.

% A principal característica das Florestas Aleatórias é sua habilidade de construir um conjunto de árvores de decisão com amostras de treinamento selecionadas aleatoriamente. Além disso, em cada divisão de um nó durante a construção da árvore, a seleção dos atributos é realizada com base em um subconjunto aleatório de atributos, diferentemente das árvores de decisão que consideram todos os atributos disponíveis. Essa aleatoriedade ajuda a aumentar a diversidade entre as árvores, resultando em um modelo mais robusto e menos propenso ao sobreajuste.

% As Florestas Aleatórias são particularmente apreciadas por sua simplicidade e eficácia, sendo capazes de lidar com dados de alta dimensão e fornecer estimativas de importância de variáveis, o que pode ser útil para a seleção de características. Além disso, elas podem ser utilizadas para tarefas de classificação e regressão, tornando-as uma ferramenta versátil no arsenal de técnicas de aprendizado de máquina \cite{gasparetto2022survey}.

% \cite{gasparetto2022survey} destacam a eficácia das Florestas Aleatórias na classificação de textos, notando que elas oferecem um bom desempenho mesmo em cenários onde o número de características é significativamente maior que o número de amostras de treinamento. Isso se deve à sua capacidade de reduzir a variância sem aumentar significativamente o viés, equilibrando assim o compromisso entre viés e variância.

% Em resumo, as Florestas Aleatórias representam uma abordagem poderosa e flexível para o problema de classificação de textos, com a vantagem adicional de fornecer insights sobre a importância das características utilizadas para a tomada de decisão.

% \subsection{Redes Neurais}

% Redes Neurais têm demonstrado desempenho significativo na modelagem de documentos e frases para a classificação de textos. Existem dois tipos principais de redes neurais utilizadas para essas tarefas: a Rede Neural Convolucional (CNN) e a Rede Neural Recorrente (RNN), \cite{bhavani2021review}

% \subsubsection{Rede Neural Convolucional (CNN)}

% A CNN utiliza filtros em diferentes janelas, com tamanhos variados, para as características dos documentos. Esses filtros são usados para classificar as características, gerando vetores de tags de palavras. A camada de entrada extrai diferentes características usando um kernel ou filtro, que é uma convolução. A convolução utiliza o produto escalar de vetores, exibindo um resultado como zero quando o texto pertence à mesma classe. Vetores de palavras embutidos são usados para encontrar a similaridade entre vetores, e esta filtragem é utilizada para contar o número de filtros na convolução, dependendo das necessidades do usuário. Após a convolução, o filtro gera múltiplas características, que são então processadas por uma função de ativação, exibindo a saída com uma relação não linear sobre a saída. Essas convoluções são reduzidas dimensionalmente para diminuir a complexidade e o tempo computacional, \cite{bhavani2021review}

% \subsubsection{Rede Neural Recorrente (RNN)}

% A RNN é um modelo sequencial que lida com sequências de documentos de comprimento variável e dependências de longo prazo. Estas redes utilizam a rede de memória de curto e longo prazo (LSTM) para extrair sequências de características de tags de nível superior.

% Ambas as redes, CNN e RNN, são fundamentais na classificação de textos, oferecendo métodos robustos para lidar com a complexidade e variabilidade dos dados de texto. A capacidade das CNNs de paralelizar bem e lidar com textos longos, em contraste com a habilidade das RNNs de gerenciar dependências semânticas de longo prazo, destaca a diversidade de aplicações e a eficácia dessas redes neurais na classificação de textos, \cite{bhavani2021review}

% \subsection{Aprendizado por Comitê (Ensemble Learning)}

% Métodos de aprendizado de conjunto, também conhecidos como métodos de integração, agregam os resultados de múltiplos algoritmos para obter melhor desempenho e interpretação. Esses métodos são notáveis por sua capacidade de combinar as forças e compensar as fraquezas de diferentes modelos de aprendizado de máquina, resultando em uma solução mais robusta e confiável.

% Os métodos de ensemble são divididos em várias subcategorias, sendo as mais populares o bagging e o boosting. O bagging, também referido como métodos de agregação de bootstrap, calcula a média da saída de múltiplos classificadores que são treinados independentemente em subconjuntos do conjunto de dados de treinamento (amostragem com reposição). Este processo melhora a precisão e a estabilidade do modelo, sendo as florestas aleatórias (Random Forests) um exemplo comum dessa abordagem \cite{gasparetto2022survey}.

% Por outro lado, o boosting treina sequencialmente classificadores fracos com o objetivo de criar um ensemble forte. Cada classificador fraco repondera os pontos de dados com base em sua própria precisão, dando maior peso aos exemplos classificados incorretamente e menor aos corretamente classificados. Assim, os sucessores do classificador fraco focam nos pontos de dados "difíceis" que foram previamente classificados de forma errada. O AdaBoost é um exemplo clássico de algoritmo de boosting \cite{gasparetto2022survey}.

% Esses métodos de ensemble são fundamentais para melhorar a robustez e a acurácia dos modelos de classificação, reduzindo o overfitting e fornecendo meios para lidar com conjuntos de dados complexos e desafiadores. O sucesso dos métodos de ensemble reside na sua capacidade de combinar diversas estratégias de aprendizado para formar um modelo mais poderoso e adaptável a diferentes tipos de problemas de classificação.


% \subsection{Gradient Boosting} Essa é uma técnica de classificação baseada em múltiplos modelos de aprendizado de máquina fracos. Permite trabalhar com conjuntos de alta dimensionalidade. 

% \subsection{Modelos de Linguagem Pré-Treinados} Esses são modelos de linguagem pré-treinados em grandes conjuntos de dados e que podem ser facilmente utilizados para tarefas de processamento de linguagem natural, como classificação de texto. Alguns exemplos de modelos de linguagem pré-treinados incluem o BERT, o GPT e o XLNet. 

% Modelos baseados em instâncias: Esses são modelos de classificação de texto que aprendem a classificar novas instâncias de texto comparando-as com exemplos de texto rotulados previamente. Isso pode ser útil em situações em que o conjunto de dados de treinamento é pequeno ou quando é difícil obter rótulos para o conjunto de dados. 
